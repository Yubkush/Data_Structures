...
class UFNode{
    ...
    public:
        ...
        long double getFixer(){return fixer;}
        UFNode* getParent(){return parent;}
        void setGroup(Group* group){this->group = group;}
        void setFixer(long double fixer){this->fixer = fixer;}
...
class UnionFind{
    ...
    public:
         ...
         */
        void Union(int group_a, int group_b, long double factor);
        //returns company name
        int Find(int company_id);
        long double getCompanyValue(int company_id);
        //O(1)
